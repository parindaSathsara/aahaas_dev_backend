...
class BookingController extends Controller
{
    ...
    public function checkingAvailability(Request $request, $id)
    {
        ...
        try {
            ...
            if ($inventory_query > 0) {
                if ($ChildCount >= 1) {
                    // return $ChildCount;
                    if (in_array('4-5', $childBoard)) {
                        $sql_query = DB::table('tbl_hotel')
                            ->where('tbl_hotel.id', $id)
                            ->where('tbl_hotel_room_rate.travel_startdate', '<=', $CheckIn)
                            ->where('tbl_hotel_inventory.max_adult_occupancy', '<=', $AdultCount)
                            ->where('tbl_hotel_inventory.max_child_occupancy', '<=', $ChildCount)
                    ...
                    } else if (in_array('6-11', $childBoard)) {
                        $sql_query = DB::table('tbl_hotel')
                            ->where('tbl_hotel.id', $id)
                            ->where('tbl_hotel_room_rate.travel_startdate', '<=', $CheckIn)
                            ->where('tbl_hotel_inventory.max_adult_occupancy', '<=', $AdultCount)
                            ...
                            ->where('tbl_hotel_room_rate.child_withno_bed_age', '=', '6-11')->where('tbl_hotel_inventory.room_category', $RoomCategory)
                            ->join('tbl_hotel_room_rate', 'tbl_hotel.id', '=', 'tbl_hotel_room_rate.hotel_id')->join('tbl_hotel_inventory', 'tbl_hotel.id', '=', 'tbl_hotel_inventory.hotel_id')
                            ->select(
                                'tbl_hotel.hotel_name',
                                'tbl_hotel_room_rate.travel_startdate',
                        ...
                        return response()->json([
                            'status' => 404,
                            'query_result' => 'No Availability for given criteria'
                        ]);
                    }
                } else {
                    $sql_query = DB::table('tbl_hotel')
                    ...
                    // return array_chunk($hotelArray, count($meals));


                    // foreach ($meals as $mealTp) {
                    //     if ($mealTp == "HB") {
                    //         foreach ($hotelArrayHB as $hb) {
                    ...
                    // $devidedArray = array_chunk($hotelArray, count($meals));
    ...
    public function bookingAmendmentEmail($amendBookId)
    {
        ...
        try {
            Mail::send('Mails.ReciptBody', $dataSet, function ($message) use ($userEmail, $resevationNumber, $pdf) {
                $message->to($userEmail);
